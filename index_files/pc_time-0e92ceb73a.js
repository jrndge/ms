...
//                  }
        ...
        */
        that.init = function (ss) {
            ...
            that.buttons.cancel = { text: s.setText, css: ' -s', handler: that.select };
            that.buttons.clear = { text: s.clearText, css: 'dwb-cl', handler: that.clear };

            that._isInput = $elm.is('div');
            ...
            if (that._isInput) {
                ...
                $elm.on('change.dw', function () {
            		...
            		if( $elm.attr('id') == 'start_time' ){
					if( $('#end_time').text() !='' ){
						if( $elm.val() < $('#end_time').text() ){
							$('#'+$elm.attr('id')+'').text($elm.val())
						}else{
							$elm.val( '' );
							layer.open({
							    content: '开始时间要小于结束时间'
				...
				}else if( $elm.attr('id') == 'end_time' ) {
					if( old_endtime == ''  ){
						if( $('#start_time').text() !='' ){
							if( $elm.val() > $('#start_time').text() ){
								$('#'+$elm.attr('id')+'').text($elm.val())
							}else{
								$elm.val('');
								layer.open({
